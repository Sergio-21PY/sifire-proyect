...
export const formContainer = {
  ...
  padding: '1.5rem', boxShadow: C.shadow, marginBottom: '1.5rem', border: `1px solid ${C.border}`,
};
...
export const inputStyle = (hasError) => ({
  width: '100%', padding: '0.6rem 0.8rem', borderRadius: C.radiusSm, boxSizing: 'border-box',
  border: `1px solid ${hasError ? C.red : C.border}`,
  backgroundColor: C.bg, color: C.textPrimary, fontSize: '0.95rem', outline: 'none',
});
...
export const submitButton = (loading) => ({
  backgroundColor: loading ? '#7f1d1d' : C.red, color: '#fff', border: 'none',
  padding: '0.7rem 1.5rem', borderRadius: C.radiusSm, fontWeight: 600,
...
export const asignacionesBadge = (n) => ({
  backgroundColor: n > 0 ? '#1e3a5f' : C.surfaceAlt,
  color: n > 0 ? '#93c5fd' : C.textMuted,
  padding: '0.2rem 0.7rem', borderRadius: '999px', fontSize: '0.8rem', fontWeight: 600,
});
export const estadoBadge = (estado) => ({
  backgroundColor: estado === 'ACTIVO' ? '#052e16' : '#1e293b',
  color: estado === 'ACTIVO' ? '#4ade80' : '#f87171',
  padding: '0.2rem 0.7rem', borderRadius: '999px', fontSize: '0.8rem', fontWeight: 600,
});
export const actionButton = {
  backgroundColor: C.surfaceAlt, color: C.textMuted, border: `1px solid ${C.border}`,
  padding: '0.3rem 0.8rem', borderRadius: '6px', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer',
};